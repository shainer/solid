...
{
    if (static_cast<HalDevice *>(m_device)->prop("info.interfaces").toStringList().contains("org.freedesktop.Hal.Device.Volume.Crypto")) {
        ...
        return static_cast<HalDevice *>(m_device)->prop("volume.is_mounted").toBool();
    }
}

QString StorageAccess::filePath() const
{
    QString result = static_cast<HalDevice *>(m_device)->prop("volume.mount_point").toString();

    if (result.isEmpty()) {
        QStringList mountpoints
            = FstabHandling::possibleMountPoints(static_cast<HalDevice *>(m_device)->prop("block.device").toString());
...
bool StorageAccess::isIgnored() const
{
    ...
    if (static_cast<HalDevice *>(m_device)->prop("volume.ignore").toBool() || isLocked) {
        return true;
    }

    const QString mount_point = StorageAccess(static_cast<HalDevice *>(m_device)).filePath();
    const bool mounted = static_cast<HalDevice *>(m_device)->prop("volume.is_mounted").toBool();
    if (!mounted) {
        return false;
     ...
     * are useless to him.
     */
    Solid::Device drive(static_cast<HalDevice *>(m_device)->prop("block.storage_device").toString());
...
bool StorageAccess::setup()
{
    ...
    } else if (FstabHandling::isInFstab(static_cast<HalDevice *>(m_device)->prop("block.device").toString())) {
...
void StorageAccess::slotDBusReply(const QDBusMessage &/*reply*/)
{
    if (m_setupInProgress) {
        ...
        HalDevice drive(static_cast<HalDevice *>(m_device)->prop("block.storage_device").toString());
        if (drive.prop("storage.drive_type").toString() != "cdrom"
                && drive.prop("storage.requires_eject").toBool()) {

            QString devnode = static_cast<HalDevice *>(m_device)->prop("block.device").toString();
...
bool StorageAccess::callHalVolumeMount()
{
    ...
    QString fstype = static_cast<HalDevice *>(m_device)->prop("volume.fstype").toString();
    QStringList halOptions = static_cast<HalDevice *>(m_device)->prop("volume.mount.valid_options").toStringList();

    QString alternativePreferred = static_cast<HalDevice *>(m_device)->prop("volume.fstype.alternative.preferred").toString();
    if (!alternativePreferred.isEmpty()) {
        QStringList alternativeFstypes = static_cast<HalDevice *>(m_device)->prop("volume.fstype.alternative").toStringList();
        if (alternativeFstypes.contains(alternativePreferred)) {
            fstype = alternativePreferred;
            halOptions = static_cast<HalDevice *>(m_device)->prop("volume.mount." + fstype + ".valid_options").toStringList();
        }
    }
